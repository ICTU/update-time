...
In a workflow or action definition, the actions in the `uses:` references. Actions referenced by a branch (e.g. `@main`) or as a local action without an `@` don't resolve to a version, so they are not updated.
...
Both are indirect cases. When the version is derived from the project's Dockerfile, only the staleness of the base image is reported. The entry itself is never checked then, and a `stale` marker on it is reported as redundant. A version taken from Docker Hub is checked against the newest Node or Python release there.
...
Neither is checked: both name a version of a runtime rather than a release of a package, which is not something OSV matches an advisory against. A `vulnerable` scope on a `.python-version` entry is reported as redundant.
