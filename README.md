...
Each type links to its own section under [Details per dependency type](#-details-per-dependency-type), which covers the files and dependencies it updates, and how pinning, the cooldown, staleness, yanks, vulnerabilities, archival, and markers apply to it.
...
Update-time also reports what updating cannot put right: a dependency whose newest release is years old, a version its maintainer withdrew, a version a security advisory names, and a dependency its maintainer has archived. These warnings are informational, so they change no file and do not affect the exit status.
...
An archived dependency is one its maintainer has declared finished: no further release is expected. Update-time warns when a dependency's source declares it archived:
...
No command-line option switches the archival check off. `--stale-after 0` does not switch it off either, so a run with the staleness check off still asks the sources above about the projects they report on. A marker silences the warning rather than the lookup, and only a bare `# update-time: ignore` skips both.
...
By default the marker holds a reference back from version updates, the [staleness](#-stale-dependencies) check, the [yank](#-yanked-dependencies) check, the [vulnerability](#-vulnerable-dependencies) check, and the [archival](#-archived-dependencies) check. Add a bracketed scope to narrow it to just one:
...
So `# update-time: ignore[update]` keeps a deliberately pinned reference frozen while still telling you when the project behind it has gone quiet or its version was withdrawn, `# update-time: ignore[stale]` silences a staleness warning you've acknowledged without freezing the version, and `# update-time: ignore[yanked]` does the same for a yank you have decided to live with. `# update-time: ignore[vulnerable]` silences the vulnerability warning for one you have assessed, while the reference keeps updating. `# update-time: ignore[archived]` silences the archival warning for a dependency you have decided to keep using. A reason can still follow the scope, for example `# update-time: ignore[update] (pinned until the 3.13 migration)`.
...
Archival can only be observed where the dependency's source publishes an archival signal — of the references that accept a marker, `requirements.txt` requirements, GitHub Actions, and pre-commit hooks (see [Archived dependencies](#-archived-dependencies)). On a Docker image, a jsDelivr URL, a `.python-version` entry, or a Node engine the scope can never suppress anything, so it is reported as redundant:
...
A `requirements.txt` requirement that pins no exact version is checked for archival all the same, so an `ignore[archived]` on it holds a warning back. That directive is not reported as redundant, where an `ignore[yanked]` and an `ignore[vulnerable]` are.
...
For each dependency type, this chapter answers the same questions: what files, dependencies, and versions are updated, how pinning, the cooldown, staleness, yanks, vulnerabilities, and archival are handled, and how markers can be placed. Two sections cover a pair of types that behave alike, so there are fewer sections than types.
...
PyPI publishes a project status, which Update-time reads for each dependency a Python file declares, whether or not it pins an exact version: archival is a fact about the project, so the package's name alone is enough to look it up. A compiled or hash-pinned `requirements.txt` is the exception: Update-time skips it whole, so its requirements are neither updated nor checked. Two kinds of dependency are left out as well, since neither names a PyPI project to read a status from: one that points at a URL or a git repository, and one uv resolves through a `[tool.uv] sources` entry, such as a path or a workspace member. A `requirements.txt` requirement whose project is archived is reported unless an `ignore[archived]` marker silences that warning. A `pyproject.toml` or inline script metadata dependency is reported as well, but takes no marker to silence that warning.

#### Markers

Write a marker for a `requirements.txt` pin inline, on the pin's own line, as in `humanize==4.15.0  # update-time: ignore`; see [Controlling updates and warnings per reference](#-controlling-updates-and-warnings-per-reference) for the directives and where they go. A requirement that pins no exact version is checked for staleness and archival. It can carry a marker as well: `humanize>=4  # update-time: ignore[stale<1095]` warns once that package's newest release is more than three years old. A `yanked` or `vulnerable` scope on a requirement that pins no exact version is reported as redundant, since these checks need a pinned version. As Update-time does not update such a requirement, a `cooldown` and a bound are also reported as redundant. Update-time reads no markers in `pyproject.toml` or inline script metadata. Opt a requirement in one of those out of updating by pinning it with a maximum or non-`==` specifier instead, for example `package<=3.12`.
...
The npm registry publishes no archival signal, so a `package.json` dependency is not checked. What npmjs.com shows as a deprecated package is a deprecation on the release its `latest` tag points at, which says something about that release rather than about the project.
...
GitHub has no yank concept, so neither an action nor a hook `rev:` is checked for one, and a `yanked` scope on either is reported as redundant.
...
Both are checked against the `archived` flag GitHub publishes for a repository. Archival is a fact about the repository rather than about one of its versions, so the flag is read whatever the reference names: a version, a branch, or a commit. GitHub publishes no reason beside the flag, so the warning gives none. An action or a hook whose repository is archived is reported unless an `ignore[archived]` marker silences that warning.
...
Neither source has a yank concept, so neither is checked for one, and a `yanked` scope on either is reported as redundant.
...
Neither is checked. Both take their version from a Docker base image or from Docker Hub, and no OCI registry publishes an archival signal. An `archived` scope on either is reported as redundant.
...
An OCI registry has no yank concept, so an image is not checked for one, and a `yanked` scope on an image reference is reported as redundant.
...
An image is not checked, because no OCI registry publishes an archival signal. An `archived` scope on an image reference is reported as redundant.
...
A jsDelivr URL is not checked, since the npm registry publishes no archival signal. An `archived` scope on a URL is reported as redundant.
