...
requirements.txt, PEP 723 inline script metadata, package.json, pom.xml,
.python-version, Dockerfiles, CircleCI configs, .gitlab-ci.yml, Docker Compose
files, Helm charts, devcontainer configs, YAML files under .github/, .pre-
commit-config.yaml, and Sphinx config. A cooldown period holds back releases
that are too fresh to trust.
...
options:
  ...
  --exclude-path PATHS  comma-separated list of directories, relative to the
                        ...
                        build, target, node_modules, __pycache__, and hidden
                        folders. Directories are matched by relative path, not
                        by name: --exclude-path vendor excludes vendor/ at the
                        root but not sub/vendor/. Directories that don't exist
                        are ignored and logged at log-level WARNING. Absolute
...
3. Review the log and the changes.
4. Commit the changes and open a pull request.
5. Let your tests and checks run in CI to confirm nothing is broken before merging.
...
| [Maven dependencies](#maven-dependencies) | `pom.xml` | the repositories the project's Maven build resolves from | delegated to Maven's [versions plugin](https://www.mojohaus.org/versions/versions-maven-plugin/) |
...
Update-time works on both version pins and hash pins. It moves a version pin forward, and takes the most precise spelling the source has for the version it lands on. And it adds a hash pin to any reference that can hold one.

#### Floating image tags

A floating image tag does not pin a version: what the project pulls is whatever the registry serves under that tag at that moment. Update-time replaces it with the version and digest it serves at the time of the run, which is the image the project pulls today:
...
| :-------------- | :-------------------- |
| [Python dependencies](#python-dependencies) | nothing: a pin names a version, and Update-time writes no hash beside it |
| [npm and pnpm dependencies](#npm-and-pnpm-dependencies) | nothing: the integrity hashes live in the package manager's lock file |
| [Maven dependencies](#maven-dependencies) | nothing: a pom names a version, and Maven records an artefact's checksum beside the artefact |
...
| [Maven dependencies](#maven-dependencies) | Update-time, for the artefacts Maven Central serves | the version's publication date in the repository |
...
Update-time reports what an update cannot fix:
...
Update-time warns when a dependency's newest release is older than a threshold. That may mean the project was abandoned or superseded. The threshold defaults to **365 days**, and `--stale-after DAYS` sets it. Pass `--stale-after 0` to disable the check. A reference that takes a marker can set a threshold of its own, or leave the check out altogether, and either wins over whatever `--stale-after` says (see [Controlling updates and warnings per reference](#-controlling-updates-and-warnings-per-reference)). For example, Update-time reports a pin whose newest release is well over a year old as:
...
Update-time checks the dependencies it asks a source about for staleness, against the date that source reports:
...
| [Maven dependencies](#maven-dependencies) | none: Maven Central does not report a withdrawal |
| [Node engine version](#node-engine-version-and-python-version) | none: its source does not record a yank |
| [Python version](#node-engine-version-and-python-version) | none: its source does not record a yank |
| [Docker images](#docker-images) | none: its source does not record a yank |
| [GitHub Actions](#github-actions-and-pre-commit-hooks) | none: its source does not record a yank |
| [Pre-commit hooks](#github-actions-and-pre-commit-hooks) | none: its source does not record a yank |
...
Update-time checks the version the run leaves the reference on. So it warns about a vulnerability the run updated into, and not about one the run updated away from. It does not check transitive dependencies. Auditing the transitive dependencies in a lock file is what `uv audit`, `pip-audit`, `npm audit`, and `pnpm audit` are for.

A reference that takes a marker can set a risk level of its own, silence one advisory, or leave the check out altogether (see [Controlling updates and warnings per reference](#-controlling-updates-and-warnings-per-reference)).
...
| [Maven dependencies](#maven-dependencies) | none: OSV is asked about no Maven artefact |
...
| [GitHub Actions](#github-actions-and-pre-commit-hooks) | none: OSV matches no version against its advisories for actions |
...
| [jsDelivr npm URLs](#jsdelivr-npm-urls) | none: the npm registry does not publish an archival signal |
...
| Bare `ignore` | the verb with no bracket at all, which applies to every scope it can without naming one | `# update-time: ignore` |
...
Only the `ignore` verb silences an advisory. The `allow` verb would keep that advisory's warning and drop the warning about every other advisory, hiding the vulnerabilities you have not assessed. So the language does not offer it.
...
A `package.json` dependency takes no marker. Opt one out by declaring an upper bound or an exact version instead (see [npm and pnpm dependencies](#npm-and-pnpm-dependencies)). A `pom.xml` dependency takes none either, because Maven updates it (see [Maven dependencies](#maven-dependencies)).

#### Confirming a marker was understood

Run with `--log-level DEBUG` to confirm a marker is recognised. Update-time logs every recognised marker, every update it holds back, every pin kept floating, and every warning silenced, each on a line of its own. Update-time reports a marker it recognised as:
...
```console
WARNING Redundant update-time directive ignore[stale<90] for ghcr.io/astral-sh/uv in Dockerfile:2: this dependency's source does not report a publication date to measure staleness against
```

Three kinds of reference get that warning. An image on a registry other than Docker Hub does, since only Docker Hub reports a push date. So Update-time does not report the same marker on a Docker Hub image. Update-time does not ask a registry about a reference a file accounts for itself, so that reference gets the warning too. A CircleCI machine-executor image and an image a Docker Compose file builds are such references. And a runtime version that follows the project's Dockerfile gets the warning as well, whether it is a `.python-version` entry or a Node engine. The staleness reported for it is the base image's, not its own.

#### The `cooldown` scope

Update-time reports the `cooldown` scope as redundant for a reference whose source does not report a publication date to measure a cooldown against:

```console
WARNING Redundant update-time directive ignore[cooldown<30] for python in .python-version:2: this dependency's source does not report a publication date to measure a cooldown against
```

Three kinds of reference get that warning. A runtime version that follows the project's Dockerfile gets it, whether it is a `.python-version` entry or a Node engine. Update-time already applied its cooldown when it updated the base image. An image on a registry other than Docker Hub gets the warning too. Only Docker Hub reports a push date to measure a cooldown against, so Update-time does not report the same marker on a Docker Hub image. Update-time does not ask a registry about a reference a file accounts for itself, so that reference gets the warning as well. A CircleCI machine-executor image and an image a Docker Compose file builds are such references.

Update-time reports a `cooldown` on a `pyproject.toml` or inline script metadata dependency as redundant, since uv takes a cooldown per run rather than per dependency (see [Setting a cooldown period](#setting-a-cooldown-period)). It reports one on a `requirements.txt` requirement that pins no exact version too. PyPI dates its releases, but Update-time does not resolve an update for such a requirement, so the cooldown holds nothing back.
...
```console
WARNING Redundant update-time directive ignore[yanked] for humanize in docs/requirements.txt:12: this requirement does not pin a version to check for a yank
```
...
Update-time still checks a reference marked `ignore[vulnerable]`, and silences what it finds rather than warning about it. So it reports the marker as redundant for a version that the check clears. A suppression can outlive the vulnerability it was written for, and the check is what tells you it did:
...
```console
WARNING Redundant update-time directive ignore[archived] for python in Dockerfile:2: this dependency's source does not publish an archival signal
```
...
An `ignore` or an `ignore[update]` keeps nothing floating either, since Update-time never pins a reference whose update it holds back. Update-time reports the `allow[floating-pin]` directive as redundant whether or not the tag floats:
...
A [bound](#bounding-an-update) can be redundant in two ways:
...
Update-time updates a dependency uv resolves through a `[tool.uv] sources` entry like any dependency in the file. Such an entry names a path, a workspace member, or another index. Here too uv resolves the dependency, and Update-time only writes the version uv reports. PyPI does not serve a release for such a dependency, nor for one pointing at a URL or a git repository. So none of the warnings below applies to either.
...
Update-time skips a package published only as `.egg` files, since PyPI no longer accepts files named that way.
...
Update-time checks each exact pin a Python file declares against OSV's PyPI advisories.
...
Update-time reports a `requirements.txt` requirement whose project is archived, unless an `ignore[archived]` marker silences that warning. It reports a `pyproject.toml` or inline script metadata dependency the same way.

#### Markers

Write a marker for a `requirements.txt` pin inline, on the pin's own line. A `pyproject.toml` dependency and an inline script metadata dependency take one in either placement. See [Controlling updates and warnings per reference](#-controlling-updates-and-warnings-per-reference) for the directives and where they go.

An `ignore[update]` holds the dependency's update back, so Update-time leaves that pin as the file wrote it and reports no new version for it. The pin is what constrains uv, so a pin Update-time declines to rewrite is the version uv keeps resolving. A dependency that pins no exact version has no pin to freeze, since uv both resolves and records its version. A name a file declares twice is steered per declaration, so a marker steers the line it sits on and leaves the other line alone.

Update-time checks a requirement that pins no exact version for staleness and archival, so such a requirement can carry a marker as well: `humanize>=4  # update-time: ignore[stale<1095]` warns once that package's newest release is more than three years old.
...
npm and pnpm handle deprecated versions themselves when they resolve an update, so Update-time does not check a `package.json` dependency for one.

#### Vulnerable dependencies

Update-time does not check a `package.json` dependency. It declares a range rather than a version, and the lock file records the version that range resolves to. Update-time does not read that lock file.

#### Archived dependencies

The npm registry does not publish an archival signal, so Update-time does not check a `package.json` dependency. What npmjs.com shows as a deprecated package is a deprecation on the release its `latest` tag points at. That says something about the release rather than about the project.

#### Markers

A `package.json` dependency takes no marker, because npm and pnpm update it rather than Update-time rewriting its lines. Opt one out by declaring an upper bound or an exact version instead.

### Maven dependencies

#### What files are updated?

Update-time looks for `pom.xml` files recursively from the starting path.

#### What dependencies are updated?

Update-time reads every `<dependency>` element the pom declares, wherever it sits: under `<dependencies>`, under `<dependencyManagement>`, and in a `<profile>`. A dependency that declares no `<version>` of its own, such as one a parent pom manages, is left out, since this pom holds no version to advance.

Update-time delegates updating Maven dependencies to Maven. The Maven it runs is the project's own `./mvnw` wrapper when one sits beside the pom, and the `mvn` on your path where the project ships none. In each pom's own directory, it runs two goals of the [versions plugin](https://www.mojohaus.org/versions/versions-maven-plugin/). Update-time names the plugin's version in each goal it runs, so which version of the plugin runs is Update-time's decision rather than the scanned project's. Both goals run in one invocation. The `use-latest-releases` goal advances a dependency whose `<version>` element holds a version. The `update-properties` goal advances a dependency whose `<version>` element names one of the pom's properties, by advancing that property. Update-time reports such a dependency at the property's line, which is where the change landed. Two dependencies that name the same property therefore get a report each, both pointing at that line.
...
The versions plugin picks the newest release of each dependency, and adopts no snapshot. Update-time excludes the pre-releases, which the plugin would otherwise adopt, since Maven reads a release candidate as a release.

#### Pinning

Update-time adds no hash pin to a Maven dependency, since a pom has nowhere to hold one: a `<version>` element names a version. The checksum of an artefact lives beside that artefact, in the repository Maven downloads it from.
...
Maven Central does not report a withdrawal, so Update-time never warns that a Maven dependency is yanked.

#### Vulnerable dependencies

Update-time does not ask OSV about a Maven dependency, so it never warns that one is vulnerable.
...
A `pom.xml` dependency takes no marker, because Maven updates it rather than Update-time rewriting its lines.
...
GitHub does not record a yank, so Update-time checks neither an action nor a hook `rev:` for one.

#### Vulnerable dependencies

Update-time checks neither. OSV does hold advisories for actions, but their affected entries enumerate no versions. So a question to OSV about a version of an action returns nothing, however that version is spelled. Silence there says nothing about whether the action is safe. A hook repository is not an OSV package at all.
...
A version taken from Docker Hub honours the cooldown through the Node or Python image tag's push date. A version that instead follows the project's Dockerfile needs no cooldown of its own, since Update-time already applied one when it updated the base image.

#### Stale dependencies

Both are indirect cases. When the version comes from the project's Dockerfile, Update-time reports the staleness of the base image alone. It checks neither the entry nor the engine itself then. It checks a version taken from Docker Hub against the newest Node or Python release there.

#### Yanked dependencies

Neither source has a yank concept, so Update-time checks neither for one.

#### Vulnerable dependencies

Update-time checks neither. Both name a version of a runtime rather than a release of a package, which is not something OSV matches an advisory against.

#### Archived dependencies

Update-time checks neither. Both take their version from a Docker base image or from Docker Hub, and no OCI registry publishes an archival signal.
...
Update-time ignores an image whose tag it cannot read, so a reference through a `{{ ... }}` template or `${VAR}` variable substitution gets no digest. Update-time does not ask a registry about a reference a file accounts for itself, so that reference gets none either. A CircleCI machine-executor image (the `image:` under a `machine:` key, such as `ubuntu-2204:2024.01.1`) and an image a Docker Compose file builds are such references.

#### Cooldown

Update-time adopts a newer tag only once it is past the cooldown, provided Docker Hub hosts the image. Other registries (`ghcr.io`, `mcr.microsoft.com`, …) do not expose a publication date, so Update-time updates images there without a cooldown. Pinning a floating tag adopts no newer image, so the cooldown holds nothing back there (see [Floating image tags](#floating-image-tags)).

#### Stale dependencies

Staleness is measured against the image's newest release, on Docker Hub only, since other registries do not expose a publication date.

Update-time reports no staleness for a reference whose tag the registry does not serve. That tag names no image in the repository, so the image's newest release says nothing about the reference. An image a pipeline builds without pushing it is the common case, and a deleted or mistyped tag reads the same way.

Two kinds of tags are dated all the same. A tag naming a version is, whether or not the registry serves it. So is a tag listed further down the repository's tag list than Update-time reads, since what the registry serves under such a tag is unknown.

#### Yanked dependencies

An OCI registry does not record a yank, so Update-time does not check an image for one.

#### Vulnerable dependencies

Update-time does not check an image, because OSV has no ecosystem for container images. Reporting the vulnerabilities of the packages inside an image is what an image scanner is for.

#### Archived dependencies

Update-time does not check an image, because no OCI registry publishes an archival signal.
...
Update-time checks the version in the URL against the npm registry's deprecations. It warns about a URL the run leaves on a deprecated version.
...
Update-time does not check a jsDelivr URL, since the npm registry does not publish an archival signal.
