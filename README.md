...
- **Docker images, GitHub Actions, `requirements.txt` dependencies, and jsDelivr npm URLs** — Update-time enforces the cooldown itself, based on each image tag's push date and each release's publication date. For a GitHub Action version that was tagged without a GitHub release, the tagged commit's date is used.
...
- **Inline**, on the reference's own line (in YAML files, `requirements.txt`, and `devcontainer.json`):
