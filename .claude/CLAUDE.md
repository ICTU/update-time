...
Run anything you point at the repository with `uv run`, never the system `python3`. That covers a `python3 - <<EOF` heredoc that edits one file and restores it, as much as a tree-wide rewrite.

A bulk rewrite can destroy uncommitted work, a `sed` or `perl -pi` one-liner as much as a script. Name the files to rewrite explicitly rather than globbing the tree, and keep the substitution idempotent, so a crash halfway through can be re-run rather than undoing what already landed. Read the diff it produced before running anything else: a regex that spans a pair of delimiters can start at one you meant and end at a later one you didn't, and a substring can match a longer line that merely starts the same way. Stub and restore whole lines. A target that is only part of a line rewrites the rest of that line too, and being unique is no protection, since the `Edit` tool accepts any fragment that matches once.
...
1. New tests follow the conventions of the nearest existing test for the same kind of behaviour, unless that test breaks a rule below. A table of cases loops with `subTest` naming each case, rather than repeating an assertion or a helper call.
...
8. Settle a "could we do X?" design question by trying X and reporting what the checker or the suite says, not by reasoning about it: the tools name the exact errors, and the count tells us how much the alternative would cost. This holds for questions you mean to ask me too — try it first, so the question reaches me with numbers instead of an estimate. When the deciding tool only runs in CI, try each option locally anyway and hand me what the local tools say plus what stays unverified, rather than an estimate of what CI would say. And don't announce a comparison you then don't run. A probe is evidence only when it would fail if the answer were the other way, so establish that before reporting what it showed: a rule that matched nothing passes exactly like a rule that holds.
9. Check a claim against the code or the tools before stating it — "nothing covers this yet", "this brings the file under the coverage gate" — since a sibling module, an existing test, or one run usually settles it. A review finding is a claim too, and a challenged claim is one to check again, not to repeat. So is an instruction in an issue: check what it assumes against the code before carrying it out.
...
When every candidate test passes, review the increment before proposing the self-improvement session: the whole of what it changed, against the same criteria, reported as its own numbered list. A finding can span cycles and show up in no single cycle's diff, such as a helper the second cycle duplicated, or an abstraction the third one made obvious.
...
- A user-visible change gets a changelog entry under `[Unreleased]`, one line naming the behaviour and linking the issue. The detail belongs in the README, not in the changelog: the README names the behaviour and shows the message it produces, and leaves out a worked example of what that message already shows. When a change lands over several cycles, update both in the cycle that lands the behaviour they describe, so neither ever documents a state the code isn't in.
