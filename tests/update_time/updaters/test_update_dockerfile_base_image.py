...
@mock_docker_hub_auth
class UpdateDockerfileTest(RegistryRequestsMixin, LoggingTestCase):
    ...
    def test_pin_unpinned_base_image(self, mock_glob: Mock):
        """Test that a base image referenced by tag only is pinned with the latest tag and digest."""
        self.requests.side_effect = mock_docker_registry(docker_tag("1.22", DIGEST2))
    ...
    def test_stage_alias_is_preserved_when_pinning(self, mock_glob: Mock):
