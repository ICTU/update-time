...
from update_time.references import rewrite as references_rewrite
...
class UpdateManifestImagesTest(registry.ImageUpdaterTestMixin):
    ...
    @kills(
        Mutation(
            ...
            "    del compose_file_contents\n    return tag_getter_excluding(set(), Unserved.BUILT_IMAGE)\n",
            "the images a Compose file builds are looked up on a registry like any other",
        ),
        Mutation(
            manifest_images,
            '        if isinstance(service, dict) and "build" in service and isinstance(service.get("image"), str)\n',
            '        if isinstance(service, dict) and "build" not in service '
            'and isinstance(service.get("image"), str)\n',
            "the image a service runs is read as one the file builds, and the one it builds as one it runs",
        ),
        Mutation(
            references_rewrite,
            "        if latest.unserved is not None:\n"
            "            self.logger.unserved_reference(reference, latest.unserved)\n"
            "            return match.string\n",
            "        if latest.unserved is not None:\n            return match.string\n",
            "a reference the file itself accounts for is left alone without a word, so nothing says why",
        ),
    )
    def test_every_reference_to_a_built_image_is_left_alone(self):
        ...
        built = Unserved.BUILT_IMAGE
        self.assert_unserved_reference_logged("acme/api", "1.2.3", Location(mock_manifest, 4), built)
        self.assert_unserved_reference_logged("acme/api", "1.2.3", Location(mock_manifest, 6), built)
        self.assert_unserved_reference_logged("acme/db", "2.0.0", Location(mock_manifest, 9), built)
    ...
    @kills(
        Mutation(
            ...
            "every image a service names is read as one the file builds, so none of them is ever updated",
        ),
        Mutation(
            manifest_images,
            '        if isinstance(service, dict) and "build" in service and isinstance(service.get("image"), str)\n',
    ...
    @kills(
        Mutation(
            ...
            '        return reason if pinned.name in {name.split(":", maxsplit=1)[0] for name in images} else None\n',
    ...
    @kills(
        Mutation(
            ...
            "    if floats is False:\n",
            "a reference no registry serves is reported as one whose pin does not float, which its tag may well do",
        )
    )
    def test_floating_pin_marker_on_a_built_image_is_reported_as_unserved(self):
        """Test that `allow[floating-pin]` on a built image says no registry serves it, not that it does not float."""
        self.requests.side_effect = mock_docker_registry(docker_tag("1.3.0", DIGEST))
        ...
        self.assert_redundant_directive_logged(
            Reason.REFERENCE_NOT_SERVED, "acme/api", Location(mock_manifest, 4), "allow[floating-pin]"
        )
...
class ScannedManifestsTest(unittest.TestCase):
    ...
    def scan(self) -> tuple[Mock, Mock]:
        """Run the updater over no files, and return the file type each of its two passes is given."""
        with (
    ...
    def test_docker_compose_files_are_scanned(self):
        ...
        update_yaml_files, _update_files = self.scan()
        self.assertEqual(update_yaml_files.call_args_list[0].args, (DOCKER_COMPOSE_FILES,))
