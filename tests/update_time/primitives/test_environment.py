...
_INT_VAR: EnvVar[int] = EnvVar("_UPDATE_TIME_TEST_INT", default=7, parse=int)
_BOOL_VAR: EnvVar[bool] = EnvVar(
...
class EnvVarTest(unittest.TestCase):
    ...
    def test_custom_parse_and_serialize_round_trip(self):
        ...
        with patch_environ():
            _BOOL_VAR.set(value=True)
            self.assertTrue(_BOOL_VAR.get())
            _BOOL_VAR.set(value=False)
            self.assertFalse(_BOOL_VAR.get())
