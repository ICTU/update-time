...
class WithinCooldownTest(unittest.TestCase):
    ...
    def test_configured_cooldown(self):
        ...
        timestamp = datetime.now(UTC) - timedelta(days=COOLDOWN.default + 1)
        self.assertFalse(within_cooldown(timestamp))
        with patch_environ({COOLDOWN.name: str(COOLDOWN.default + 2)}):
    ...
    def test_just_within_cooldown(self):
        ...
        self.assertTrue(within_cooldown(datetime.now(UTC) - timedelta(days=COOLDOWN.default, hours=-1)))

    def test_old_timestamp(self):
        """Test that a timestamp from before the cooldown period is not within cooldown."""
        self.assertFalse(within_cooldown(datetime.now(UTC) - timedelta(days=COOLDOWN.default, hours=1)))
