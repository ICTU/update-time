...
def update_pyproject_tomls() -> int:
    ...
    warn_about_stale_dependencies(files, _newest_releases, LOG.warn_if_stale)
    return 0


def _newest_releases(pyproject_toml: Path) -> Iterable[tuple[str, DependencyVersion]]:
    """Yield each declared `==` pin as a (name, newest PyPI release) pair, for the staleness check."""
    return (
        (name, get_latest_version(name, version))
        for name, version in pyproject_toml_format.pinned_versions(pyproject_toml).items()
    )
