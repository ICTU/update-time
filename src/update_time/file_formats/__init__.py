...
Two tiers, one package (for now): the modules split into generic serialization formats (`yaml.read` parses any
YAML; likewise the tomllib/json parsing behind `pyproject_toml.read` and `package_json.read`) and package-manifest
specifics (`pyproject_toml`'s `tool_key`/`set_tool_key`/`rewrite_pinned_versions`, `requirements_txt.is_compiled`).
