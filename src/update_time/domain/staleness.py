...
def is_stale(published: datetime | None, threshold: int) -> bool:
    ...
    A threshold that switches the check off makes nothing stale, and an unknown publication date never counts as
    stale either. Whole days are compared (via `days_since`), so a fractional day over the threshold is not stale
    yet.
    """
    if threshold == NO_STALENESS_CHECK or published is None:
        return False
