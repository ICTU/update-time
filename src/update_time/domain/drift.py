"""Whether a pin has drifted, and whether the reference adopts it or only warns.

A pin can stop matching what it points at while the version stays put: an image tag re-pushed under the same name
serves a new digest, a mutable version tag can be moved onto another commit, and a declared integrity hash can
disagree with the one the CDN serves. Adopting a change silently would defeat the pin, so it is adopted only when
the reference opted in — per reference with an `# update-time: allow[pin-drift]` marker, or repo-wide with
`--allow-pin-drift`. Adoption covers a digest or a commit only: refusing content that doesn't match is the whole
point of a hash, so a mismatched one is always left to be corrected. Both rules live here so every kind of pin
...
# Private channel that passes --allow-pin-drift from the CLI to the updater subprocesses: whether a drifted pin —
# a re-pushed image digest, or the commit a moved version tag now points at — should be adopted repo-wide.
ALLOW_PIN_DRIFT = EnvVar(
    "_UPDATE_TIME_ALLOW_PIN_DRIFT",
    default=False,
...
def pin_drifted(resolved: str, pinned: str) -> bool:
...
def drift_cause(marker: Marker) -> str | None:
    ...
    when both it and the repo-wide flag apply; the pin-drift opt-in is among them. The cause is named for the log
    message reporting the adoption.
    """
    if marker.allow_drift:
        return f"update-time: {marker.raw_marker(Verb.ALLOW)}"
    return "--allow-pin-drift" if ALLOW_PIN_DRIFT.get() else None
